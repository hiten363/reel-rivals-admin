...
import BulkCoupanModal from './Modals/BulkCoupanModal';

const Coupon = ({ notify }) => {
  const { getCoupans, deleteCoupan, undoCoupan, getDraws } = useMain();
  ...
  const columns = [
    ...
    {
      name: 'User',
      selector: row => <div>{row.user?.name}</div>,
      sortable: true,
      wrap: true
    },
    {
    ...
    //       if (row.status === "true") {
    //         setId(row._id);
    //         setMsg("Are you sure you want to delete selected faq?");
    //         document.getElementById('deleteModal').classList.toggle('hidden');
    //       }
    //       else {
    //         let ans = await undoCoupan({ id: row._id });
    //         if (ans.status) {
    //           notify('success', 'Coupon recovered successfully');
  ...
  const getData = async () => {
    setLoadFlag(true);
    const ans = await getCoupans(value.status, value.query, page, perPage);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoadFlag(true);
    console.log(value);
    const ans = await getCoupans(value.status, value.query, 1, perPage, '', value.isAvailableCoupan ? '' : value.isRevealedCoupan ? 'false' : value.isRewardCoupan ? 'true' : 'undefined', value?.event);
    setTotalRows(ans.count);
    setPage(1);
    setData(ans.data);
    setLoadFlag(false);
  ...
  return (
    <>
      ...
      <BulkCoupanModal setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
      <EditCoupanModal data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
            ...
            <form className="flex items-start justify-end px-5 pt-3" onSubmit={handleSubmit}>
              <div className='flex items-center justify-start flex-wrap w-full'>
                <div className='mr-2 w-[23%] min-w-[220px] mb-2'>
                  <Input label="Search Vouchers .." name="query" onChange={handleChange} value={value.query} />
                </div>
                ...
                {/* <div className='w-[23%] min-w-[250px] ml-2.5 mb-2'>
                  <Switch label="Show Used Vouchers Only" className='text-sm font-semibold' name='isRevealedCoupan' onChange={(e)=>{handleChange(e, 'isRevealedCoupan', true)}} />
                </div>

                <div className='w-[23%] min-w-[250px] ml-2.5 mb-2'>
                  <Switch label="Show Available Vouchers Only" className='text-sm font-semibold' name='isAvailableCoupan' onChange={(e)=>{handleChange(e, 'isAvailableCoupan', true)}} />
                </div> */}
