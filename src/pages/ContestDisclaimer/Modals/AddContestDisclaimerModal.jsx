...
const AddContestDisclaimerModal = (props) => {
  const { postContestDisclaimer } = useMain();
  ...
  const handleSubmit = async (e) => {
    ...
    const ans = await postContestDisclaimer({ ...value, desc: `<div className="quill-component">${value.desc.richText}</div>` });
    console.log(ans);
    if (ans.status) {
      setValue({
        desc: {
  ...
  return (
    <>
      <div id="addContestDisclaimerModal" tabIndex="-1" className="fixed cus-modal top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
...
export default AddContestDisclaimerModal;
