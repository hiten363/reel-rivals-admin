...
const Blog = ({ notify }) => {
  const { getBlogs, deleteBlog, undoBlog } = useMain();
  // console.log('yes');
      ...
      selector: row => <div className="flex justify-center">
        ...
        <div onClick={async () => {
          if (row.status === "true") {
            ...
            let ans = await undoBlog({ id: row._id });
            if (ans.status) {
              notify('success', 'Blog recovered successfully');
  ...
  const getData = async () => {
    setLoadFlag(true);
    const ans = await getBlogs('', '', value.status, value.query, page, perPage);
    console.log(ans);
    setData(ans.data);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(value);
    const ans = await getBlogs('', '', value.status, value.query ,1, perPage);
    setTotalRows(ans.count);
    setPage(1);
    console.log(ans);
    setData(ans.data);
        ...
        <Card>
          <CardHeader variant="gradient" color="gray" className="mb-8 p-6">
            <div className="flex items-center justify-between">
              ...
              }} children="Add Blog +">Add Blog +</Button>
            </div>
          </CardHeader>
