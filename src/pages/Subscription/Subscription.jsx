...
const Subscription = ({ notify }) => {
    ...
    },
    {
      ...
      selector: row => <div className="flex justify-center">
        ...
        <div onClick={async () => {
          if (row.status === "true") {
            setId(row._id);
            setMsg("Are you sure you want to delete selected subscription?");
            document.getElementById('deleteModal').classList.toggle('hidden');
          }
          else {
            let ans = await undoSubscription({ id: row._id });
            if (ans.status) {
              notify('success', 'Subscription recovered successfully');
  ...
  const handleDelete = async () => {
    console.log(id);
    const ans = await deleteSubscription(id);
  ...
  return (
    <>
      ...
      <EditSubscriptionModal data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
        ...
        <Card>
          <CardHeader variant="gradient" color="gray" className="mb-8 p-6">
            <div className="flex items-center justify-between">
              ...
              }} children="Add Subscription +">Add Plan +</Button>
            </div>
          </CardHeader>
