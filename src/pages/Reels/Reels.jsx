...
import { Button, Card, CardBody, CardHeader, Typography, select } from '@material-tailwind/react';
...
const Reels = ({ notify }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(value);
    const ans = await getVideos('', value.category, value.contest, '', value.user, '', value.query, value.status, 1, perPage);
    setTotalRows(ans.count);
    setPage(1);
    console.log(ans);
    setData(ans.data);
