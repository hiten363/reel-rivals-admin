...
const Category = ({ notify }) => {
    ...
    },
    {
      ...
      selector: row => <div className="flex justify-center">
        ...
        <div onClick={async () => {
          if (row.status === "true") {
            setId(row._id);
            setMsg("Are you sure you want to delete selected Category?");
            document.getElementById('deleteModal').classList.toggle('hidden');
          }
          else {
            let ans = await undoCategory(row._id);
            if (ans.status) {
              notify('success', 'Category recovered successfully');
  ...
  return (
    <>
      <AddCategoryModal setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
      <EditCategoryModal data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
