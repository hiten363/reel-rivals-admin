...
const EditContactModal = ({ data, setRefreshFlag, refreshFlag, notify }) => {
  const { updateContact, deleteContactImage } = useMain();
  ...
  const deleteImg = async () => {
    ...
    const ans = await deleteContactImage(prevImage.id);
    console.log(ans);
    if (ans.status) {
      setPrevImage('');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const ans = await updateContact({ ...value, desc: desc.richText, tags: temp });
    if (ans.status) {
  ...
  return (
    <>
      <div id="editContactModal" tabIndex="-1" className="fixed top-0 left-0 right-0 z-50 cus-modal hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
            ...
            <div className="p-6 space-y-6">
              <form onSubmit={handleSubmit}>
                <div className="bus-form">
                  <h4 className="text-xl text-center mb-4 font-bold">Update Contact details</h4>
                  ...
                  <div className='text-right'>
                    <button type="submit" className="text-white bg-primary focus:ring-4 focus:outline-none focus:ring-purple-200 font-medium rounded-sm text-sm w-full sm:w-auto px-5 py-2.5 text-center "><span>Submit</span></button>
                  </div>
                </div>
              </form>
            </div>
