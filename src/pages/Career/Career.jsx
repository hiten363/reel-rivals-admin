...
import AddCareerModal from './Modals/AddCareerModal';
...
const Career = ({ notify }) => {
  const { getCareers, deleteCareer, undoCareer } = useMain();
  // console.log('yes');
    ...
    },
    {
      ...
      selector: row => <div className="flex justify-center">
        ...
        <div onClick={async () => {
          if (row.status === "true") {
            setId(row._id);
            setMsg("Are you sure you want to delete selected career?");
            document.getElementById('deleteModal').classList.toggle('hidden');
          }
          else {
            let ans = await undoCareer({ id: row._id });
            if (ans.status) {
              notify('success', 'Career recovered successfully');
  ...
  const getData = async () => {
    setLoadFlag(true);
    const ans = await getCareers('', value.status, value.query, page, perPage);
    console.log(ans);
    setData(ans.data);
  ...
  const handleDelete = async () => {
    console.log(id);
    const ans = await deleteCareer(id);
  ...
  return (
    <>
      <AddCareerModal setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
      <EditCareerModal data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
        ...
        <Card>
          <CardHeader variant="gradient" color="gray" className="mb-8 p-6">
            <div className="flex items-center justify-between">
              ...
              }} children="Add Career +">Add Career +</Button>
            </div>
          </CardHeader>
