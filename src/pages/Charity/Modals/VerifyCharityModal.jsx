  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (response.status) {
        notify(
          'success',
          `Charity ${verified ? 'verified' : 'unverified'} successfully`
        );
        refreshData();
        handleClose();
      } else {
        notify(
          'error',
          response.message || 'Failed to update charity verification status'
        );
      }
    } catch (error) {
      ...
      notify(
        'error',
        'An error occurred while updating charity verification status'
      );
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <DialogBody divider className="space-y-4">
        <div>
          ...
          <div className="mt-2 space-y-2">
            ...
            <div>
              ...
              <Typography variant="paragraph">
                {charity.createdBy?.userName || 'Unknown'}
              </Typography>
            </div>
            ...
            <div>
              ...
              <Typography variant="paragraph">
                {new Date(charity.createdAt).toLocaleString()}
              </Typography>
            </div>
        ...
        <div className="border-t pt-4">
          ...
          >
            Are you sure you want to {charity.verified ? 'unverify' : 'verify'}{' '}
            this charity?
          </Typography>
