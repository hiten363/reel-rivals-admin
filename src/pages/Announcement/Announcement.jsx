...
const Announcement = ({ notify }) => {
  const { getAnnouncements, deleteAnnouncement, undoAnnouncement } = useMain();
  // console.log('yes');
  ...
  const getData = async () => {
    setLoadFlag(true);
    const ans = await getAnnouncements(page, perPage);
    setData(ans.data);
  ...
  const handleDelete = async () => {
    console.log(id);
    const ans = await deleteAnnouncement(id);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(value);
    const ans = await getAnnouncements(1, perPage);
    setTotalRows(ans.count);
    setPage(1);
    console.log(ans);
    setData(ans.data);
  ...
  return (
    <>
      <AddAnnouncementModal setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
      <EditAnnouncementModal data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
        ...
        <Card>
          <CardHeader variant="gradient" color="gray" className="mb-8 p-6">
            <div className="flex items-center justify-between">
              ...
              }} children="Add Announcement +">Add Announcement +</Button>
            </div>
          </CardHeader>
