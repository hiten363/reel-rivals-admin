...
const AddStatisticsModal = (props) => {
  const { postStatistics } = useMain();
  ...
  const handleSubmit = async (e) => {
    ...
    const ans = await postStatistics({ ...value });
    console.log(ans);
    if (ans.status) {
      setValue({
        count1: '',
  ...
  return (
    <>
      <div id="addStatisticsModal" tabIndex="-1" className="fixed cus-modal top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
            ...
            <div className="p-6 space-y-6">
              <form onSubmit={handleSubmit}>
                <div className="bus-form">
                  <h4 className="text-xl text-center mb-4 font-bold">Enter Statistics details</h4>
