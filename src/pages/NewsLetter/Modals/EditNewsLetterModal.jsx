...
const EditNewsLetterModal = ({ data, setRefreshFlag, refreshFlag, notify }) => {
  ...
  const deleteImg = async () => {
    ...
    const ans = await deleteNewsLetterImage(prevImage.id);
    console.log(ans);
    if (ans.status) {
      setPrevImage('');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const ans = await updateNewsLetter({ ...value, desc: desc.richText, tags: temp });
    if (ans.status) {
  ...
  return (
    <>
      <div id="editNewsLetterModal" tabIndex="-1" className="fixed top-0 left-0 right-0 z-50 cus-modal hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
            ...
            <div className="p-6 space-y-6">
              <form onSubmit={handleSubmit}>
                <div className="bus-form">
                  <h4 className="text-xl text-center mb-4 font-bold">Update NewsLetter details</h4>
