...
const EditPrivacyModal = ({ flag, data, setRefreshFlag, refreshFlag, notify }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const ans = await updatePrivacy({ ...value, desc: `<div className="quill-component">${desc.richText}</div>` });
    if (ans.status) {
  ...
  return (
    <>
      <div id="editPrivacyModal" tabIndex="-1" className="fixed top-0 left-0 right-0 z-50 cus-modal hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        {!loadFlag ? <Spinner1 /> : <div className="cus-modal1 relative w-full h-full max-w-7xl md:h-auto">
          <div className="relative bg-white rounded-lg shadow ">
            <div className="flex items-center justify-between p-5 border-b rounded-t ">
              <h3 className="text-xl font-medium text-gray-900 ">
                Update Privacy and Conditions
              </h3>
