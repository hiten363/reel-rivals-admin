...
import AddPrivacyModal from './Modals/AddPrivacyModal';
...
const Privacy = ({ notify }) => {
  const { getPrivacys } = useMain();
  // console.log('yes');
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: row => <div className="flex justify-center">
        <div onClick={() => {
          setData1(row);
          document.getElementById('editPrivacyModal').classList.toggle('hidden');
        ...
        {/* <div onClick={async () => {
          if (row.status === "true") {
            ...
            let ans = await undoPrivacy({ id: row._id });
            if (ans.status) {
              notify('success', 'Privacy recovered successfully');
  ...
  //   console.log(id);
  //   const ans = await deletePrivacy(id);
  ...
  return (
    <>
      <AddPrivacyModal setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
      <EditPrivacyModal flag={flag} data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
        ...
        <Card>
          <CardHeader variant="gradient" color="gray" className="mb-8 p-6">
            <div className="flex items-center justify-between">
              <Typography variant="h6" color="white">Manage Privacy Policy</Typography>
