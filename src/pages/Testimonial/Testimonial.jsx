...
const Testimonial = ({ notify }) => {
  const { getTestimonials, deleteTestimonial, undoTestimonial } = useMain();
  // console.log('yes');
    ...
    },
    {
      ...
      selector: row => <div className="flex justify-center">
        ...
        <div onClick={async () => {
          if (row.status === "true") {
            setId(row._id);
            setMsg("Are you sure you want to delete selected testimonial?");
            document.getElementById('deleteModal').classList.toggle('hidden');
          }
          else {
            let ans = await undoTestimonial({ id: row._id });
            if (ans.status) {
              notify('success', 'Testimonial recovered successfully');
  ...
  const getData = async () => {
    setLoadFlag(true);
    const ans = await getTestimonials('', value.status, value.query, page, perPage);
    console.log(ans);
    setData(ans.data);
  ...
  const handleDelete = async () => {
    console.log(id);
    const ans = await deleteTestimonial(id);
  ...
  return (
    <>
      <AddTestimonialModal setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
      <EditTestimonialModal data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
        ...
        <Card>
          <CardHeader variant="gradient" color="gray" className="mb-8 p-6">
            <div className="flex items-center justify-between">
              <Typography variant="h6" color="white">Manage Testimonials</Typography>

              <Button color="red" onClick={() => {
                document.getElementById('addTestimonialModal').classList.toggle('hidden');
              }} children="Add Testimonial +">Add Testimonial +</Button>
            </div>
          </CardHeader>
