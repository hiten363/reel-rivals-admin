...
const GiftCard = ({ notify }) => {
  const { getGiftCards, deleteGiftCard, undoGiftCard } = useMain();
  // console.log('yes');
    ...
    },
    {
      ...
      selector: row => <div className="flex justify-center">
        ...
        <div onClick={async () => {
          if (row.status === "true") {
            setId(row._id);
            setMsg("Are you sure you want to delete selected gift card?");
            document.getElementById('deleteModal').classList.toggle('hidden');
          }
          else {
            let ans = await undoGiftCard({ id: row._id });
            if (ans.status) {
              notify('success', 'GiftCard recovered successfully');
  ...
  const handleDelete = async () => {
    console.log(id);
    const ans = await deleteGiftCard(id);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(value);
    const ans = await getGiftCards(value.status, value.query, 1, perPage);
    setTotalRows(ans.count);
    setPage(1);
    console.log(ans);
    setData(ans.data);
  ...
  return (
    <>
      {/* <AddGiftCardModal setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} />
      <EditGiftCardModal data={data1} setRefreshFlag={setRefreshFlag} refreshFlag={refreshFlag} notify={notify} /> */}
          ...
          <CardHeader variant="gradient" color="gray" className="mb-8 p-6">
            <div className="flex items-center justify-between">
              ...
              {/* <Button color="red" onClick={() => {
                document.getElementById('addGiftCardModal').classList.toggle('hidden');
              }} children="Add GiftCard +">Add GiftCard +</Button> */}
            </div>
          </CardHeader>
