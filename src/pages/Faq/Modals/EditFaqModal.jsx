...
const EditFaqModal = ({ data, setRefreshFlag, refreshFlag, notify }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const ans = await updateFaq({ ...value });
    if (ans.status) {
      notify('success', ans.message);
      setRefreshFlag(!refreshFlag);
      document.getElementById('editFaqModal').classList.toggle('hidden');
    }
    else {
  ...
  return (
    <>
      <div id="editFaqModal" tabIndex="-1" className="fixed top-0 left-0 right-0 z-50 cus-modal hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
            ...
            <div className="p-6 space-y-6">
              <form onSubmit={handleSubmit}>
                <div className="bus-form">
                  <h4 className="text-xl text-center mb-4 font-bold">Update Faq details</h4>
