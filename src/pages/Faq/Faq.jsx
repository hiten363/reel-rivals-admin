...
const Faq = ({ notify }) => {
  const { getFaqs, deleteFaq, undoFaq } = useMain();
  // console.log('yes');
      ...
      selector: row => <div className="flex justify-center">
        ...
        <div onClick={async () => {
          if (row.status === "true") {
            ...
            let ans = await undoFaq({ id: row._id });
            if (ans.status) {
              notify('success', 'Faq recovered successfully');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(value);
    const ans = await getFaqs(value.status, value.query, 1, perPage);
    setTotalRows(ans.count);
    setPage(1);
    console.log(ans);
    setData(ans.data);
