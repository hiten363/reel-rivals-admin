...
const ContestUser1 = ({ notify }) => {
  const { getUsers, updateUserStatus, getContests, assignUserBadge, unassignUserBadge } = useMain();
  ...
  const columnsWithOG = [
    ...
    {
      name: 'Assigne OG Badge',
      selector: row => !row?.achievements?.includes('OG') ? (
        <button type="button" onClick={() => {
          handleAssignBadge(row._id);
        }} className="text-white text-xs bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded px-3 py-1 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Assign</button>
      ) : (
        <button type="button" onClick={() => {
          handleUnassignBadge(row._id);
        }} className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-xs px-3 py-1 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Unassign</button>
      ),
      grow: 1
    },
    {
  ...
  const getData = async () => {
    setLoadFlag(true);
    const [ans1, ans2] = await Promise.allSettled([getUsers(value.status, value.role, value.query, page, perPage, '', contestId), getContests(contestId)])
    setTotalRows(ans1.count);
    setData([...ans1.value.data.sort((a, b) => {
      return new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
    })]);
    setContestInfo(ans2.value?.data?.[0] || {});
    setLoadFlag(false);
  };
  ...
  const handleAssignBadge = async (userId) => {
    const ans = await assignUserBadge({ userIds: [userId], badges: ['OG'] });
    if (ans.status) {
      notify('success', 'OG Badge assigned successfully');
      setRefreshFlag(!refreshFlag);
    }
    else {
  ...
  const handleUnassignBadge = async (userId) => {
    const ans = await unassignUserBadge({ userIds: [userId], badges: ['OG'] });
    if (ans.status) {
      notify('success', 'OG Badge unassigned successfully');
      setRefreshFlag(!refreshFlag);
    }
    else {
      notify('error', ans.message);
      ...
      <div className="mt-12 mb-8 flex flex-col gap-12">
        <Card>
          ...
          <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
            ...
            <DataTable
              columns={contestInfo?.isOGContest ? columnsWithOG : columns}
              data={data}
              striped={true}
